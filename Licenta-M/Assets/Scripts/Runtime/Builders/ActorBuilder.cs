...
namespace MF
{
	public class ActorBuilder
	{
		...
		public ActorBuilder WithAnima<T>() where T : Anima, new()
		{
			anima = controller.gameObject.AddComponent<T>();

			return this;
		}

		public ActorBuilder WithAnima(Type animaType)
		{
			if (!typeof(Anima).IsAssignableFrom(animaType))
			{
				Debug.LogError(
					$"Tried to build actor [{controller}] with an anima of type [{animaType}] but that does not inherit from {nameof(Anima)}");
				return this;
			}

			anima = (Anima)Activator.CreateInstance(animaType);
