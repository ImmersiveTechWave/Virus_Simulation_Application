...
namespace MF
{
	...
	public class Promise<PromisedT> : IPromise<PromisedT>, IPendingPromise<PromisedT>, IPromiseInfo
	{
		...
		/// <summary>
		/// Returns a promise that resolves when any of the results of the promises passed as argument pass the specified check.
		/// If no such promise exists, a promise that resolves to false is returned
		/// </summary>
		public static IPromise<bool> Any(Predicate<PromisedT> condition, params IPromise<PromisedT>[] promises)
		{
			return Any(condition, (IEnumerable<IPromise<PromisedT>>)promises);
		}
		...
		/// If no such promise exists, a promise that resolves to false is returned
		/// </summary>
		public static IPromise<bool> Any(Predicate<PromisedT> condition, IEnumerable<IPromise<PromisedT>> promises)
		{
			...
			var resultPromise = new Promise<bool>();
			resultPromise.WithName("Any");
			...
			promisesArray.Each((promise, index) =>
			{
				promise
					...
					.Then(result =>
					{
						...
						{
							// This will never happen if any of the promises erred.
					...
					.Catch(ex =>
					{
						if (resultPromise.CurState == PromiseState.Pending)
						{
							// If a promise erred and the result promise is still pending, reject it.
							resultPromise.Reject(ex);
						}
					})
					.Done();
		...
		/// Returns a promise of a collection of the resolved results that meet the specified criteria
		/// </summary>
		public static IPromise<IEnumerable<PromisedT>> Where(Predicate<PromisedT> condition,
			params IPromise<PromisedT>[] promises)
		{
			return Where(condition, (IEnumerable<IPromise<PromisedT>>)promises); // Cast is required to force use of the other Where function.
		}
		...
		/// </summary>
		public static IPromise<IEnumerable<PromisedT>> Where(Predicate<PromisedT> condition,
			IEnumerable<IPromise<PromisedT>> promises)
		{
			var promisesArray = promises.ToArray();
			if (promisesArray.Length == 0)
			{
			...
			var remainingCount = promisesArray.Length;
			var results = new List<PromisedT>(remainingCount);
			var progress = new float[remainingCount];
			var resultPromise = new Promise<IEnumerable<PromisedT>>();
			resultPromise.WithName("Where");
		...
		/// <summary>
		/// Returns a promise that resolves when the first of the promises in the enumerable argument resolves and passes the specified check.
		/// 
		/// <para>If no such promise exists, the returned promise is rejected</para>
		/// </summary>
		public static IPromise<PromisedT> FirstOrDefault(Predicate<PromisedT> condition, PromisedT defaultValue, bool stopOnException = true, params IPromise<PromisedT>[] promises)
		{
			return FirstOrDefault(condition, defaultValue, (IEnumerable<IPromise<PromisedT>>)promises, stopOnException); // Cast is required to force use of the other function.
		}

		/// <summary>
		/// Returns a promise that resolves when the first of the promises in the enumerable argument resolves and passes the specified check.
		/// 
		/// <para>If no such promise exists, the promise will resolve with default(PromisedT)</para>
		/// </summary>
		public static IPromise<PromisedT> FirstOrDefault(Predicate<PromisedT> condition, PromisedT defaultValue, IEnumerable<IPromise<PromisedT>> promises, bool stopOnException = true)
		{
			var promisesList = promises.ToList();
			if (promisesList.Count == 0)
			{
				throw new InvalidOperationException(
					"At least 1 input promise must be provided for Race"
				);
			}
			var defaultPromise = new Promise<PromisedT>();
			defaultPromise.Resolve(defaultValue);
			promisesList.Add(defaultPromise);

			var resultPromise = new Promise<PromisedT>();
			resultPromise.WithName("FirstOrDefault");

			var progress = new float[promisesList.Count];

			promisesList.Each((promise, index) =>
			{
				promise
					.Progress(v =>
					{
					...
					.Then(result =>
					{
						if (resultPromise.CurState != PromiseState.Pending) return;

						if (condition(result))
						{
							resultPromise.Resolve(result);
						}
						else if (index == promisesList.Count - 1)
						{
							resultPromise.Resolve(defaultValue);
						}
					})
					.Catch(ex =>
					{
						if (resultPromise.CurState == PromiseState.Pending && (index == promisesList.Count - 1 || stopOnException))
						{
							// If a promise erred and the result promise is still pending, reject it.
							resultPromise.Reject(ex);
						}
